<!-- Footer -->
<footer class="bg-slate-900 text-white pt-20 pb-10">
    <div class="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-20">
        <div class="col-span-1 md:col-span-1 lg:col-span-1">
            <a href="{{ getSiteUrl() }}" class="flex items-center gap-3 mb-6">
                <div class="h-16 w-16 rounded-2xl object-contain bg-white p-3 shadow-lg">
                    <img src="https://res.cloudinary.com/dzv1tksr2/image/upload/v1772609946/4_lqe3dl.png" alt="FeedTan Logo" class="w-full h-full object-contain">
                </div>
                <div class="flex flex-col">
                    <span class="text-lg font-black tracking-tighter text-white leading-none">FeedTan</span>
                    <span class="text-xs font-bold text-slate-400 leading-none mt-1">Community Microfinance Group</span>
                </div>
            </a>
            <p class="text-slate-400 leading-relaxed text-sm mb-6">
                Cultivating sustainable communities and growing hope through innovative food security solutions since 2009.
            </p>
            <div class="flex items-center gap-4">
                <span class="text-xs font-bold text-emerald-400">501(c)(3)</span>
                <span class="text-xs text-slate-500">|</span>
                <span class="text-xs font-bold text-emerald-400">15+ Years</span>
                <span class="text-xs text-slate-500">|</span>
                <span class="text-xs font-bold text-emerald-400">Tanzania</span>
            </div>
        </div>

        <div>
            <h4 class="font-bold mb-6 text-emerald-500">Quick Links</h4>
            <ul class="space-y-3 text-sm">
                <li><a href="{{ getSiteUrl() }}" class="hover:text-white transition-colors">Home</a></li>
                <li><a href="{{ getSiteUrl('about') }}" class="hover:text-white transition-colors">About Us</a></li>
                <li><a href="{{ getSiteUrl('campaigns') }}" class="hover:text-white transition-colors">Campaigns</a></li>
                <li><a href="{{ getSiteUrl('impact') }}" class="hover:text-white transition-colors">Our Impact</a></li>
                <li><a href="{{ getSiteUrl('get-involved') }}" class="hover:text-white transition-colors">Volunteer</a></li>
            </ul>
        </div>

        <div>
            <h4 class="font-bold mb-6 text-emerald-500">Get Involved</h4>
            <ul class="space-y-3 text-sm">
                <li><a href="{{ getSiteUrl('donate') }}" class="hover:text-white transition-colors">Make a Donation</a></li>
                <li><a href="{{ getSiteUrl('get-involved') }}" class="hover:text-white transition-colors">Volunteer Programs</a></li>
                <li><a href="{{ getSiteUrl('campaigns') }}" class="hover:text-white transition-colors">Current Campaigns</a></li>
                <li><a href="{{ getSiteUrl('contact') }}" class="hover:text-white transition-colors">Contact Us</a></li>
            </ul>
        </div>

        <div>
            <h4 class="font-bold mb-6 text-emerald-500">Contact Info</h4>
            <ul class="space-y-4 text-sm text-slate-400">
                <li class="flex items-start gap-3">
                    <i class="ph ph-map-pin text-emerald-500 mt-0.5"></i>
                    <span>
                        FeedTan Community Microfinance Group<br>
                        P.O.Box 7744, Ushirika Sokoine Road<br>
                        Moshi, Kilimanjaro, Tanzania<br>
                        <br>
                        <strong>Website:</strong> {{ getSiteUrl() }}<br>
                        <strong>Email:</strong> info@feedtan.org<br>
                        <strong>Phone:</strong> (555) 123-4567
                    </span>
                </li>
                <li class="flex items-center gap-3">
                    <i class="ph ph-globe text-emerald-500"></i>
                    <span>Online Platform: Live Website</span>
                </li>
                <li class="flex items-center gap-3">
                    <i class="ph ph-server text-emerald-500"></i>
                    <span>Server Status: Active & Running</span>
                </li>
                <li class="flex items-center gap-3">
                    <i class="ph ph-clock text-emerald-500"></i>
                    <span>Business Hours: Mon-Fri 8AM-6PM EAT</span>
                </li>
                <li class="flex items-center gap-3">
                    <i class="ph ph-shield-check text-emerald-500"></i>
                    <span>Registration: NGO Registered in Tanzania</span>
                </li>
            </ul>
        </div>
    </div>

    <div class="max-w-7xl mx-auto px-6 pt-10 border-t border-slate-800">
        <div class="flex flex-col items-center text-center gap-8">
            <div class="flex flex-wrap items-center justify-center gap-y-2 gap-x-6 text-[11px] font-bold uppercase tracking-wider text-slate-500">
                <a href="{{ getSiteUrl('terms') }}" class="hover:text-emerald-500 transition-colors">Terms & Conditions</a>
                <a href="{{ getSiteUrl('privacy') }}" class="hover:text-emerald-500 transition-colors">Privacy Policy</a>
                <a href="{{ getSiteUrl('faq') }}" class="hover:text-emerald-500 transition-colors">FAQ</a>
            </div>
            <p class="text-sm text-slate-500">© 2024 FeedTan Community Microfinance Group. All rights reserved. | Celebrating 10 Years of Service</p>
        </div>
    </div>
</footer>
